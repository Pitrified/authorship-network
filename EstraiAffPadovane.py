...
def processaAm(pfAffRAW, chunkStart, chunkSize, sIDAffPad):
  try:
...
def processaPAAm(pfPapAutAffRAW, chunkStart, chunkSize):
  try:
    #carica solo le linee da processare
...
def estraiAffPadovane(pfPapAutAffRAW, pfPapAffPad):
  """
  ...
  for chunkStart, chunkSize in chunkMyFile(pfPapAutAffRAW, roughSize):
    lresult.append(pool.apply_async(processaPAAm,(pfPapAutAffRAW, chunkStart, chunkSize) ) )
  ...
  for chunkStart, chunkSize in chunkMyFile(pfAffRAW, roughSize):
    lresult.append(pool.apply_async(processaAm,(pfAffRAW, chunkStart, chunkSize, sIDAffPad)))
...
if __name__ == '__main__':
  ...
  start = timer()
  estraiAffPadovane(pfPapAutAffRAW, pfPapAffPad)
  end = timer()
